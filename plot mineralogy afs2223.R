# afxrd2223 <- read.table("clipboard", header=T, sep="\t", stringsAsFactors=TRUE)
afxrd2223$Sample <- as.character(afxrd2223$Sample)
afxrd2223$Type <- factor(afxrd2223$Type, levels=c("Surface","Shallow core","Core"))
...
with(subset(afxrd2223,afxrd2223$Type!="Core"),
...
#             pch=21, bg="#ffffff80"))
# with(afxrd2223, text(jitter(Easting,.09), jitter(Northing,.09), labels=row.names(afxrd2223), cex=0.75))
with(afxrd2223, text(Easting, Northing, labels=row.names(afxrd2223), cex=0.75))

afxrd_clr <- afxrd2223
afxrd_clr[,8:35] <- t(apply(afxrd_clr[,8:35], MARGIN = 1,
                            FUN = function(x){log(x) - mean(log(x),na.rm=T)}))
head(afxrd_clr)
summary(afxrd_clr)
# replace NAs with nominal (low) value
for(i in 8:35){
  afxrd_clr[which(is.na(afxrd_clr[,i])),i] <- -6
}

af_min_pca <- prcomp(afxrd_clr[,c(8:35)], scale. = TRUE)
summary(af_min_pca)
...
with(af_min_pca, points(x[,1]*ff, x[,2]*ff, cex=1.75,
                        pch=c(21,22,23,24,25,21,22)[afxrd_clr$Zone],
                        bg=plasma(7, alpha=0.7)[afxrd_clr$Zone]))
legend("topleft", bty="n", inset=0.02, cex=1.2,
       title=expression(bold("Ashfield Zone")),
       legend=levels(afxrd_clr$Zone),
       pch=c(21,22,23,24,25,21,22), pt.cex=1.5,
       pt.bg=plasma(7,alp=0.7))
...
par(oma=c(0,0,0,0), mar=c(3.5,3.5,3,3), font.lab=2, font.axis=1, lend="square",
    ljoin="mitre")
ff <- 0.07
with(af_min_pca, plot(x[,1]*ff, x[,2]*ff, cex=2, asp=1,
                        pch=c(21,22,23,24,25,21,22)[afxrd_clr$Zone],
                        bg=plasma(7, alpha=0.7)[afxrd_clr$Zone],
                        xlim=c(-0.4,.7), ylim=c(-0.8,0.3),
...
legend("bottomright", bty="o", inset=c(0.02,0.1), cex=1.2,
       box.col = 8, title=expression(bold("Ashfield Zone")),
       legend=levels(afxrd_clr$Zone),
       pch=c(21,22,23,24,25,21,22), pt.cex=2,
       pt.bg=plasma(7,alp=0.7))
