...
scpassLL <- st_read("../Learning R/Acid_Sulfate_Risk_Swan_Coastal_Plain_DWER_055.shp")
...
```{r overview-map, fig.height=12.8, fig.width=20, message=FALSE, warning=FALSE, fig.cap="Map showing location of Ashfield Flats Reserve in metropolitan Perth, Western Australia.", results='hold', echo=FALSE}
palette(pal4lite)
s0 <- 2; par(oma=c(4,4,1,1)*s0, mar=c(4,4,1,1)*s0, tcl = -0.2*s0,
...
legend("bottomright", inset=c(0.02,0.02), 
       ...
       cex=0.9*s0, pch=22, pt.cex=2*s0, pt.bg=c("#ffb00080"), 
...
ns1$rsd <- paste0(signif(100*ns1$sd/ns1$mean,3),"%")
...
colnames(f0)[1] <- "Statistic"
flextable(f0) |> 
set_caption(caption = "Statistical summary of pH and EC, and Al, Ca, Fe, K, Mg, Na, P, and S
...
ns1 <- as.data.frame(cbind(ns1[,c("mean","sd","rsd","0%","50%","100%")],
                           ns0$NAs))
colnames(ns1)[NCOL(ns1)] <- "NAs"
f0 <- as.data.frame(cbind(colnames(ns1),t(ns1[1:6,])))
colnames(f0)[1] <- "Statistic"
flextable(f0) |> 
set_caption(caption = "Statistical summary of La, Ce, Nd, Gd, Y, and ∑REE
            concentrations in Ashfield Flats sediment.") |>
  align(align="right",part="all") |>
...
```{r summs-type-PS, results='hold', echo=FALSE}
...
colnames(f0)[1] <- "Stat"
align(set_caption(
  flextable(f0), caption="Basic statistics for Pond Sediment"
  ), align="right",part="all")
```
...
set_caption(align(flextable(result0), align="center",part="all"), caption = "Mean comparisons and pairwise letter comparisons from Kruskal-Wallis and pairwise Conover test for selected elements including REEs (REE = \u2211REE), by sample type, at Ashfield Flats 2019-2023.")
```
...
ppal <- scico::scico(length(brk0)-1, pal="nuuk", alpha=0.8, dir=-1)
...
```{r bubbles-REEY, fig.height=7.2, fig.width=10.2, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Map of REE+Y concentrations by location at Ashfield Flats for all sampling years.", echo=FALSE}
par(oma=c(3,3,1,1), mar=c(0,0,0,0), mgp=c(1.7,0.2,0), tcl=0.25)
plot_tiles(aftiles)
with(afr_map, polygon(wetland_E, wetland_N, border = "steelblue", col="#87CEEB80"))
axis(1);axis(2);box()
