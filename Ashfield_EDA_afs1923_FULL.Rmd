...
plot(riverLL[1], col="royalblue", add=TRUE)
...
polygon(aust_map, col = "khaki", border="sienna4", lwd=s0)
...
colnames(f0)[1] <- "Stat"
align(set_caption(flextable(f0), caption = "Statisitical summary of pH and EC, and Al, As, Ba, Ca, Ce, Co, and Cr concentrations in sediment."),
...
colnames(f0)[1] <- "Stat"
align(set_caption(flextable(f0), caption = "Statisitical summary of Fe, Gd, K, LA, Li, Mg, Mn, Mo, Na, and Nd concentrations in sediment."),
      align="right",part="all")
```
...
colnames(f0)[1] <- "Stat"
align(set_caption(flextable(f0), caption = "Statisitical summary of Ni, P, Pb, S, Sr, Th, V, Y, Zn, and ∑REE concentrations in sediment."),
      align="right",part="all")
```
...
col0 <- rep(NA,length(varz))
result0 <- data.frame(Variable=col0, KW_chi=col0, KW_p=col0,
                      Drain_Sed=col0, Lake_Sed=col0, Other=col0, Saltmarsh=col0)
for (i in 1:length(varz)) {
  kw0 <- with(data0, 
...
for(i in 1:length(varz)){
  with(afs1923surf, boxplot(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~ Type, 
                        varwidth=T, names=c("Drain","Lake","Saltmarsh","Other"),
...
for(i in 1:length(varz)){
  ...
  pwc0 <- with(afs1923surf, 
             pairwise.t.test(afs1923surf[,which(colnames(afs1923surf)==varz[i])],
                                     Type))
  mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
  shadowtext(1:4, rep(0,4), labels=mcl0$Letters, col="#003087", bg="white",
...
```{r boxplots-REE-zone, fig.height=7.2, fig.width=12, fig.align='center',fig.cap="Boxplots of REEs by sampling Zone at Ashfield Flats for all sampling years (DS = Drain sediment; Fl = Flooded; LS = Lake Sediment; SM = Saltmarsh; So = Soil).", echo=FALSE}
varz <- c("La","Ce","Gd","Nd")
palette(c("black",scico(4,pal="batlow",alpha=0.65, end=0.75),"grey","white"))
...
```{r scatterplots-sumREE-Al, fig.height=10, fig.width=15, message=FALSE, warning=FALSE, fig.align='center',fig.cap="REE-Al relationships at Ashfield Flats for all sampling years. Plots (a) and (b) are of untransformed variables, plots (c) and (d) have both axes log~10~-transformed", results='hold', echo=FALSE}
...
```{r normalized-REE-Lake-sed, fig.height=3.6, fig.width=6, fig.cap="Concentrations of YREE normalized to PAAS in lake (seasonal wetland pond) sediments over all years at Ashfield Flats.", results='hold', echo=FALSE}
require(viridis)
...
```{r, echo=FALSE}
cat("Means for each element in Lake sediment:\n"); print(meanz,digits=3)
with(plotdata, oneway.test(values ~ ind))
(pwREE <- with(plotdata, pairwise.t.test(values, ind, pool.sd = F)))
pwREE_pv <- fullPTable(pwREE$p.value)               # from rcompanion
multcompLetters(pwREE_pv)                           # from multcompView
