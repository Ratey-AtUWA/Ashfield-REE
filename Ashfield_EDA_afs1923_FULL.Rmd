...
```{r scatter-plot-matrix-untransf, fig.height=12, fig.width=12, fig.align='center',fig.cap="Scatter plot matrix for log10-transformed elements at Ashfield Flats for all sampling years.", echo=FALSE, include=FALSE}
carPalette(palette())
...
```{r spm-clr, fig.height=12, fig.width=12, fig.align='center',fig.cap="Scatter plot matrix of CLR-transformed elements at Ashfield Flats for all sampling years.", results='hold', include=FALSE, echo=FALSE}
carPalette(palette())
...
```{r scatter-plot-matrix-CLR-TE, fig.height=12, fig.width=12, fig.align='center',fig.cap="Scatter plot matrix of CLR-transformed elements at Ashfield Flats for all sampling years.", results='hold', echo=FALSE}
carPalette(palette())
...
afs1923ree <- afs1923ree[,-which(colnames(afs1923ree) %in% c("La","Ce","Nd","Gd"))]
...
par(mfrow = c(2,2), mar = c(4,4,0.5,0.5), mgp = c(2.6,0.2,0), lend=2, ljoin=1,
    cex.lab = 1.5, cex.axis = 1.25, font.lab = 2, tcl = 0.2, las = 1)
for(i in 1:length(varz)){
  with(afs1923clr, plot(REE~afs1923clr[,which(colnames(afs1923clr)==varz[i])], 
                        bg = clrz[Type], pch = seq(21,25)[Type],
                     xlab = paste(varz[i],"(clr-transformed)"), cex = 1.4, 
                     ylab = ""))
  mtext(expression(paste(bold("REE")," (clr-transformed)")),2,1.6,font=2,cex = 1.4)
  for(j in 1:nlevels(afs1923clr$Type)){
    lm0 <- lm(afs1923clr$REE ~ afs1923clr[,which(colnames(afs1923clr)==varz[i])],
              subset = afs1923clr$Type == levels(afs1923clr$Type)[j])
    v0 <- subset(afs1923clr[,which(colnames(afs1923clr)==varz[i])],
              subset = afs1923clr$Type == levels(afs1923clr$Type)[j])
    x0 <- min(v0, na.rm=T); x1 <- max(v0, na.rm=T)
    lines(c(x0,x1), c(lm0$coef[1]+lm0$coef[2]*x0,lm0$coef[1]+lm0$coef[2]*x1), 
          col = clrz[j], lwd = 3)
...
text(log10(0.5) ,7.2, pos=4, col = "red3", offset=0.15,
