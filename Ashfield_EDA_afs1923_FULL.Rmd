...
library(scico)
library(flextable)
  set_flextable_defaults(font.family = 'sans', theme_fun = "theme_booktabs", 
...
register_stadiamaps(key=read.csv("../StadiaMaps-API.csv")[1,1])
```
...
afs1923$Type <- factor(afs1923$Type, levels=c("Drain_Sed", "Pond_Sed", "Saltmarsh","Other"))
...
ispond <- with(afs1923surf, which(Zone=="N" | Zone=="NE" | Zone=="NW" | Zone=="SE" | Zone=="SW"))
afs1923surf[ispond,"Type"] <- rep("Pond_Sed", length(ispond))
...
afs1923surf$Type <- factor(afs1923surf$Type, levels=c("Drain_Sed", "Pond_Sed", "Saltmarsh","Other"))
...
```{r annotation data and palettes, include=FALSE}
afr_map <- read.csv(file=paste0(git,"afr_map_v3.csv"), stringsAsFactors = TRUE)
afr_map$SW_DBCA <- as.factor(afr_map$SW_DBCA)
afr_zones <- read.csv(file=paste0(git,"afr_zones.csv"), stringsAsFactors = TRUE)
scpassLL <- st_read("../Learn-R/Acid_Sulfate_Risk_Swan_Coastal_Plain_DWER_055.shp")
scpassLL <- st_transform(scpassLL, crs=st_crs(4326))

riverLL <- st_read("srpoly.shp")

pal4lite <- c("black", viridis::plasma(8), "white")
pal4liteTransp <- 
  c("black", scico::scico(8, alpha=0.8, palette="hawaii", beg=0.1), "white")
...
tfkey <- readLines("../thunderforest-API-key.txt")

PerthRast <- get_tiles(extent, provider="Thunderforest.Neighbourhood", # Esri.WorldImagery
                       apikey=tfkey, 
                       crop = TRUE, zoom=14)
```

```{r overview-map, fig.height=13, fig.width=20, message=FALSE, warning=FALSE, fig.cap="Map showing location of Ashfield Flats Reserve in metropolitan Perth, Western Australia.", results='hold', echo=FALSE}
palette(pal4lite)
s0 <- 2; par(oma=c(0,0,0,0)*s0, mar=c(4,4,2,1)*s0, tcl = -0.25*s0,
             mgp=c(2,0.5,0)*s0, cex.axis=1.2*s0, cex.lab=1.2*s0,
             lwd=s0, lend = "square", ljoin = "mitre", lheight=0.7,
             font.lab=2)
plot(st_coordinates(extent), asp=1.22, type="n", xaxs="i", yaxs="i", 
     xlab="Longitude (\u00B0E)", 
     ylab="Latitude (\u00B0S)")
plot_tiles(PerthRast, add=T)
plot(subset(scpassLL, scpassLL$risk_class==1)[4], pal=c("#ffb00080"), add=TRUE)
plot(riverLL[1], col="#4D6EAB", add=TRUE)
addnortharrow(border = 1, text.col=1, scale = 1.1*s0, lwd=s0,
              padin = c(0.15,0.15)*s0)
addscalebar(plotepsg = 4326, linecol=1, label.col=1, htin=0.2*s0,
            label.cex=1.25*s0, widthhint = 0.25, lwd=s0,
            padin = c(0.15,0.25)*s0)
polygon(afr_map$bound_lon, afr_map$bound_lat,
        border = "sienna", col = "sienna", lwd = 1.25*s0, density=8)
points(115.945, -31.919, pch=19, cex=1.2*s0)
lines(c(115.939, 115.945), c(-31.919,-31.919), lwd=2*s0)
shadowtext(115.939,-31.92,pos=2,labels="Ashfield\nFlats\nReserve",
           col="sienna", bg="white", font=2, cex=1.3*s0)
shadowtext(115.97,-31.93, labels="Perth\nAirport",
           col=1, bg=10, font=2, cex=1.3*s0)
shadowtext(115.865,-31.955, labels="Perth\nCBD",
           col=1, bg=10, font=2, cex=1.3*s0)
text(115.863,-31.9655, labels="Derbarl Yerrigan",
     col="#D6E0FF", font=3, cex=1.*s0)
text(115.863,-31.9685, labels="(Swan River)",
     col="#D6E0FF", font=3, cex=0.85*s0)
legend("bottomright", inset=c(0.02,0.05), 
       legend=c("High to moderate risk"),
       cex=1.1*s0, pch=22, pt.cex=2*s0, pt.bg=c("#ffb00080"), 
       bg="grey88", box.col="grey50", pt.lwd=1*s0, 
       title=expression(italic("Acid sulfate risk category")))
legend("bottomleft", bg="#ffffffb0", cex=0.6*s0, text.col=1, y.intersp=0.5,
  legend = "Map tiles: Thunderforest Neighbourhood.   Projection: WGS84 (EPSG:4326)",
  box.col="#FFFFFF00", x.intersp = 0.5) ; box(lwd=s0)
par(new=TRUE, mfrow=c(1,1), fig=c(0.08,0.3,0.68,0.92),
...
rect(par("usr")[1], par("usr")[3],
     par("usr")[2], par("usr")[4],
     col = "lightblue", border = "gray50", lwd=2*s0)
polygon(aust_map, col = "khaki", border="#4D6EAB", lwd=s0)
...
```{r sample-map, echo=FALSE, fig.align='center', fig.cap="Map of sediment sample locations at Ashfield Flats for all sampling years 2019-2023.", fig.height=9.2, fig.width=15.2, message=FALSE, warning=FALSE, echo=FALSE}
palette(pal4dark)
...
par(oma=c(0,0,0,0)*s0, mar=c(3.5,3.5,2,1)*s0, tcl = -0.25*s0,
             mgp=c(2,0.5,0)*s0, cex.axis=1.2*s0, cex.lab=1.2*s0,
             lwd=s0, lend = "square", ljoin = "mitre", lheight=0.7,
             font.lab=2)
...
with(afr_map, polygon(veg_E, veg_N, border = "#D0FFB040", lwd = 5*s0))
addnortharrow(border = 10, text.col=10, scale = 1.2*s0, padin=c(0.3,0.32))
addscalebar(plotepsg = 32750, linecol=10, label.col=10, htin=0.182*s0, 
            label.cex=1.8*s0, widthhint = 0.15, padin=c(0.4,0.3))
with(afs1923, points(Easting, Northing, 
        col = 1, bg = c(2,3,4,7,9)[Year],
        lwd=2, pch = c(21,22,23,24,25)[Year], 
        cex = s0*c(2,1.7,1.7,1.7,1.7)[Year]))
box(lwd=s0)
plot(c(0,1),c(0,1),type="n",ann=F,xaxt="n",yaxt="n",bty="n",xlab="",ylab="")
legend("top", bg="grey33", cex = 1.5*s0, text.col = 1, y.intersp = 1.4, 
       legend = levels(afs1923$Year), 
       title = expression(bold("     Sampling Year      ")), 
       pch = c(21,22,23,24,25), pt.bg = c(2,3,4,7,9), col = 1,
       pt.cex = c(2.2,1.8,1.8,1.8,1.8)*s0, pt.lwd=2)
text(-0.05,0.55,pos=4,labels="Ashfield Flats Reserve", font=2, cex=1.*s0)
text(rep(-0.05,2),c(0.45,0.35),pos=4,labels=c("Map tiles:\nESRI World Imagery",
     "CRS: UTM Zone 50S,\nWGS84 (EPSG:32750)"), cex = 1.2*s0)
legend("bottom", bg="grey33", cex = 1.25*s0, text.col = 1, y.intersp = 1.4, 
    legend = c("Seasonal\nwetland ponds","Drains",
               "Extent of native\nvegetation"), 
       title = expression(bold("Map key")), 
    pch = c(NA,NA,NA), lty = c(2,1,1), lwd = c(1.4,3,5)*s0,
    col = c("lightblue","#8080FFB0","#D0FFB040"))
```
...
## ├ pH, EC, Al, Ca, Fe, K, Mg, Na, P, and S

```{r data-summaries-pH-majors, results='hold', echo=FALSE}
ns0 <- numSummary(afs1923[,c("pH","EC","Al","Ca","Fe","K","Mg","Na","P","S")], 
                  statistics = c("mean","sd","quantiles"), quantiles=c(0,0.5,1))
ns1 <- signif(as.data.frame(ns0$table),6)
ns1$'rsd%' <- signif(100*ns1$sd/ns1$mean,3)
ns1 <- as.data.frame(cbind(ns1[,c("mean","sd","rsd%","0%","50%","100%")],
                           ns0$NAs))
colnames(ns1)[4:7] <- c("Minimum","Median","Maximum","NAs")
f0 <- data.frame(Statistic=colnames(ns1),t(ns1[1:10,]))
flextable(f0) |> 
set_caption(caption = "Statistical summary of pH and EC, and Al, Ca, Fe, K, Mg, Na, P, and S concentrations in sediment.", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6)) |>
  align(align="right",part="all") |> 
    set_formatter(pH=function(x){sprintf("%.03g",x)},
                EC=function(x){sprintf("%.0f",x)},
                Al=function(x){sprintf("%.0f",x)},
                Ca=function(x){sprintf("%.0f",x)},
                Fe=function(x){sprintf("%.0f",x)},
                K=function(x){sprintf("%.0f",x)},
                Mg=function(x){sprintf("%.0f",x)},
                Na=function(x){sprintf("%.0f",x)},
                P=function(x){sprintf("%.0f",x)},
                S=function(x){sprintf("%.0f",x)})
...
## ├ La, Ce, Nd, Gd, Y, and \u2211~4~REE

```{r data-summaries-2, results='hold', echo=FALSE}
ns0 <- numSummary(afs1923[,c("La","Ce","Nd","Gd","Y","REE")], 
                  statistics = c("mean","sd","quantiles"), quantiles=c(0,0.5,1))
...
ns1 <- as.data.frame(cbind(ns1[,c("mean","sd","rsd%","0%","50%","100%")],
                           ns0$NAs))
colnames(ns1)[4:7] <- c("Minimum","Median","Maximum","NAs")
f0 <- data.frame(Statistic=colnames(ns1),t(ns1[1:6,]))
flextable(f0) |> 
  set_header_labels(REE="∑REE") |>
  align(align="right",part="all") |> 
    set_formatter(La=function(x){sprintf("%.03g",x)},
                Ce=function(x){sprintf("%.03g",x)},
                Nd=function(x){sprintf("%.03g",x)},
                Gd=function(x){sprintf("%.03g",x)},
                Y=function(x){sprintf("%.03g",x)},
                REE=function(x){sprintf("%.03g",x)}) |>
set_caption(caption = "Statistical summary of La, Ce, Nd, Gd, Y, and ∑REE
            concentrations in Ashfield Flats sediment.", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
```

---------------------------------------------------

## ├ As, Ba, Cr, Cu, Mn, Mo, Ni, Pb, Sr, Th, V, Zn

```{r summary-traces-isqg, echo=FALSE}
elem0 <- c("As","Ba","Cr","Cu","Mn","Mo","Ni","Pb","Sr","Th","V","Zn")
summ0 <- numSummary(afs1923[,elem0], 
...
for(i in 1:length(elem0)){
  if(elem0[i] %in% ISQG$Tox){
  tabl0$DGV[i] <- 
    length(which(afs1923[,elem0[i]] > ISQG[elem0[i],"DGV"]))
  tabl0$GV_high[i] <- 
    length(which(afs1923[,elem0[i]] > ISQG[elem0[i],"GV_high"]))}
}
colnames(tabl0)[c(3:5,8:9)] <- c("min","median","max","n > DGV","n > GV-high")
ft <- data.frame(Statistic=colnames(tabl0),t(signif(tabl0,4)))
flextable(ft) |>
  width(j=c(1:7), width=c(3,rep(1.75,6)), unit="cm") |>
  set_header_labels(values=list(V1="")) |> 
    set_formatter(As=function(x){sprintf("%.03g",x)},
                Ba=function(x){sprintf("%.03g",x)},
                Cr=function(x){sprintf("%.03g",x)},
                Cu=function(x){sprintf("%.04g",x)},
                Mn=function(x){sprintf("%.04g",x)},
                Mo=function(x){sprintf("%.03g",x)},
                Ni=function(x){sprintf("%.03g",x)},
                Pb=function(x){sprintf("%.03g",x)},
                Sr=function(x){sprintf("%.03g",x)},
                Th=function(x){sprintf("%.03g",x)},
                V=function(x){sprintf("%.03g",x)},
                Zn=function(x){sprintf("%.0f",x)}) |>
  set_caption(caption="Summary statistics for trace element concentrations (mg/kg) in sediment at Ashfield Flats, 2019-2023. Abbreviations: n = number of valid observations; NAs = number of missing observations; n > DGV is number of samples exceeding Default Guideline Value; n > GV-high is number of samples exceeding the upper Guideline Value at which toxicity effects might be expected (Water Quality Australia, 2023).", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
...
```{r summs-type-DS, results='hold', echo=FALSE, paged.print=FALSE}
...
f0 <- data.frame(Stat=colnames(ns0$table),t(ns0$table))
f0[3:5,1] <- c("Minimum","Median","Maximum")
flextable(f0) |>
  align(align="right",part="all") |> 
    set_formatter(pH=function(x){sprintf("%.03g",x)},
                EC=function(x){sprintf("%.0f",x)},
                ...
                P=function(x){sprintf("%.0f",x)},
                S=function(x){sprintf("%.0f",x)},
                La=function(x){sprintf("%.03g",x)},
                Ce=function(x){sprintf("%.03g",x)},
                Nd=function(x){sprintf("%.03g",x)},
                Gd=function(x){sprintf("%.03g",x)},
                Y=function(x){sprintf("%.03g",x)}) |> 
  set_caption(caption="Basic statistics for Drain Sediment", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
...
```{r summs-type-PS, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='hold'}
d0 <- subset(afs1923, afs1923$Type=="Pond_Sed")
ns0<-numSummary(d0[,c("pH","EC","Al","Ca","Fe","P","S",
                        "La","Ce","Nd","Gd","Y")],
                  statistics = c("mean","sd","quantiles"), quantiles=c(0,.5,1))
f0 <- data.frame(Stat=colnames(ns0$table),t(ns0$table))
...
flextable(f0) |>
  align(align="right",part="all") |> 
    set_formatter(pH=function(x){sprintf("%.03g",x)},
                ...
                Gd=function(x){sprintf("%.03g",x)},
                Y=function(x){sprintf("%.03g",x)}) |> 
  set_caption(caption="Basic statistics for Pond Sediment", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
```
...
flextable(f0) |>
  align(align="right",part="all") |> 
    set_formatter(pH=function(x){sprintf("%.03g",x)},
                ...
                Gd=function(x){sprintf("%.03g",x)},
                Y=function(x){sprintf("%.03g",x)}) |> 
  set_caption(caption="Basic statistics for Saltmarsh", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
```
...
align(set_caption(
  flextable(table), caption = "Shapiro-Wilk statistics and p-values for untransformed (_orig) and transformed (_log, _pow) variables, power terms, and dip test of multimodality of log-transformed variable, from soil and sediment analysis at Ashfield Flats Reserve 2019-2023.", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6)
...
```{r plot-Na-split, fig.align='center', fig.cap="Density histograms for potentially multi-modal variables.", fig.height=5, fig.width=10, message=FALSE, warning=FALSE, echo=FALSE}
extent <- st_as_sf(x = data.frame(x = c(399860,400600), 
                                  y = c(6467920,6468400)), 
                   coords = c("x", "y"), crs = UTM50S)
aftiles <- get_tiles(extent, provider = "CartoDB.Voyager", crop = TRUE, zoom=17)
palette(pal4lite) ; s0 <- 1.5
layout(matrix(c(1,1,2,1,1,3),ncol=3,byrow=T))
par(oma=c(4,4,1,1)*s0, mar=c(0,0,0,0), mgp=c(1.7,0.3,0)*s0, tcl = -0.2*s0, 
...
addnortharrow(border = 1, text.col=1, scale = 0.8*s0, padin=c(0.1,0.3))
...
with(afs1923,points(Easting-1,Northing-2,col=c("blue","sienna")[NaFac],
                    pch = c(1,0)[NaFac], cex=c(2.2,1.8)[NaFac]))
legend("topleft", legend = c("Na < 1000mg/L", "Na > 1000mg/L"),bty="n",
  pch=c("1","2"),pt.cex=0.8,col=c("blue","sienna"),inset=0.05,y.intersp=2)
legend("topleft", legend = c("Na < 1000mg/L", "Na > 1000mg/L"),bty="n",
       pch=c(1,0),pt.cex=c(3.6,3),col=c("blue","sienna"),inset=0.05,y.intersp = 2)
with(afs1923, hist(log10(Na), freq=F, breaks = "FD", main="", yaxt="n"))
with(afs1923, lines(density(log10(Na), na.rm=T), col = "#0000a080", lwd = 2))
arrows(3,0.6,3,0.2,angle=20,length=0.15,col="#0000a0",lwd=2)
with(afs1923, hist(log10(Mo), freq=F, breaks = "FD", main="", yaxt="n"))
...
s0 <- 1
par(mar=c(0,0,0,0), oma=c(4,4,1,1)*s0, mgp=c(1.7,0.3,0)*s0, tcl = -0.2*s0, 
    lend = "round", ljoin = "round", lheight=0.7)
plot(aftiles, axes=TRUE) 
...
shadowtext(tapply(afr_map$wetland_E, afr_map$SW_DBCA, mean, na.rm=T),
     ...
     labels = paste0(levels(afr_map$SW_DBCA),"\n(",
                     c("SW","S","NW","S","N","NE","SE"),")"),
     col=1, bg="#FFFFFF10")
shadowtext(par("usr")[2],6468000, cex = 1.1, pos = 2, col="black", bg="white", 
...
plot(aftiles); axis(1);axis(2);box(which="plot")
...
for (i in 1:nlevels(afr_zones$zone)){
  ...
  polygon(zdata$zonesE, zdata$zonesN, border = "transparent", 
        col=c(2,2,5,3,3,3,5,3,4,4,3,2)[i])
}
with(afs1923, points(Easting, Northing, pch=3, cex=0.6,
           col=c(7,7,1,7,7,7,1,7,1,1,7,7)[Zone]))
...
set_caption(align(flextable(result0), align="center",part="all"), caption = "Mean comparisons and pairwise letter comparisons from Kruskal-Wallis and pairwise Conover test for selected elements including REEs (REE = \u2211REE), by simplified sampling zone, at Ashfield Flats 2019-2023.", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
```

----------------------

Comparing means (strictly mean rank sums) between sampling types (the factor 
`Type`) as this seems most appropriate. As shown in Table 
\@ref(tab:kruskal-Type), all overall effects are significant at p<0.001.
Some post-hoc pairwise comparisons show significant differences (P&le;0.05), 
differing for each variable.

```{r kruskal-Type, message=FALSE, warning=FALSE, echo=FALSE}
data0 <- st_drop_geometry(afs1923)
varz <- c("Al","Ca","Fe","K","Na","S","P",
          "As","Co","Cr","Cu","Ni","Pb","Th","Zn",
          "La","Ce","Nd","Gd","Y","REE","REEY")
col0 <- rep(NA,length(varz))
result0 <- data.frame(Variable=col0, KW_chi=col0, KW_p=col0,
                      Drain_Sed=col0, Pond_Sed=col0, Other=col0, Saltmarsh=col0)
for (i in 1:length(varz)) {
  kw0 <- with(data0, 
              kruskal.test(data0[,which(colnames(data0)==varz[i])] ~ 
                           Type))
  pwc0 <- with(data0, 
             kwAllPairsConoverTest(data0[,which(colnames(data0)==varz[i])] ~
                                     Type))
  mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
  result0[i,] <- c(varz[i],signif(kw0$statistic,4), signif(kw0$p.value,3),
                   mcl0$Letters)
  }
set_caption(align(flextable(result0), align="center",part="all"), caption = "Mean comparisons and pairwise letter comparisons from Kruskal-Wallis and pairwise Conover test for selected elements including REEs (REE = \u2211REE), by sample type, at Ashfield Flats 2019-2023.", align_with_table=F, fp_p=fp_par(text.align = "left", padding.bottom = 6))
```
...
with(afs1923, boxplot(REE ~ Zone, cex.axis=1.6, cex.lab=2.2, cex = 1.5, 
     col = c(2,2,5,3,3,3,5,3,4,4,3,2), xlab="Sampling Zone",ylab="\u2211REE"))
...
legend(0,1, bty = "n", inset = 0.01, cex = 1.8, 
       legend = c("Drain","Pond","Saltmarsh","Mixed/other"), pch = 22,
       pt.bg = c(2,3,4,5), pt.cex = 4.5)
text(-0.1, 0.25, pos=4, cex=1.4, labels=paste0("CMD = Chapman Drain\n",
      "KMD = Kitchener Drain\n", "LP = Limestone path\n",
...
with(afs1923, boxplot(REEY ~ Zone, cex.axis=1.6, cex.lab=2.2, cex = 1.5, 
     col = c(2,2,5,3,3,3,5,3,4,4,3,2), xlab="Sampling Zone",ylab="\u2211REEY"))
...
with(afs1923, boxplot(Al ~ Zone, cex.axis=1.6, cex.lab=2.2, cex = 1.5, 
     col = c(2,2,5,3,3,3,5,3,4,4,3,2), xlab="Sampling Zone",ylab="  Al"))
...
legend(0,1, bty = "n", inset = 0.01, cex = 1.8, 
       legend = c("Drain","Pond","Saltmarsh","Mixed/other"), pch = 22,
       pt.bg = c(2,3,4,5), pt.cex = 4.5)
text(-0.1, 0.24, pos=4, cex=1.4, labels=paste0("CMD = Chapman Drain\n",
      "KMD = Kitchener Drain\n", "LP = Limestone path\n",
      "N = North wetland pond\n", "NE = North-east wetland pond\n",
      "NW = North-west wetland pond\n", "S = South wetlands / side drain\n",
      "SE = South-east wetland pond\n", "SM = Saltmarsh (east of CMD)\n",
...
```{r REE-bubbles, fig.height=7.2, fig.width=10.2, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Map of REE concentrations by location at Ashfield Flats for all sampling years.", echo=FALSE}
...
axis(1);axis(2);box()
mtext("Easting (m)",1,1.2*s0,font=2,cex = 1.2*s0)
mtext("Northing (m)",2,1.2*s0,font=2,cex = 1.2*s0)
legend("bottomright", bty="o", cex=0.8,
  legend="Map tiles: CartoDB Voyager, Projection: UTM Zone 50S, WGS84 (EPSG:32750)",
       box.col="#00000000", bg="#ffffff40", y.intersp = 0.5)
addnortharrow()
...
# ppal <- rev(UWAcolgrad(length(brk0)-1, alpha=0.6, sat=F))
ppal <- scico::scico(length(brk0)-1, pal="hawaii", alpha=0.8, dir=-1)
...
rect(xy[1]+0.035*(xy[2]-xy[1]), xy[3]+0.75*(xy[4]-xy[3]),
     xy[1]+0.205*(xy[2]-xy[1]), xy[3]+0.99*(xy[4]-xy[3]),
...
```{r bubbles-REEY, fig.height=7.1, fig.width=10.2, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Map of REE+Y concentrations by location at Ashfield Flats for all sampling years.", echo=FALSE}
par(oma=c(3,3,1,1), mar=c(0,0,0,0), mgp=c(1.7,0.2,0), tcl=0.25)  ;  s0 <- 1
plot_tiles(aftiles)
with(afr_map, polygon(wetland_E, wetland_N, border = "steelblue", col="#87CEEB80"))
axis(1);axis(2);box()
...
ff <- (0.033 * (par("usr")[4]-par("usr")[3]))/sqrt(max(afs1923$REEY,na.rm=T))
afs1923$bubcats <- cut(afs1923$REEY, breaks = brk0)
ppal <- scico::scico(length(brk0)-1, pal="lapaz", alpha=0.7, dir=-1)
...
rect(xy[1]+0.03*(xy[2]-xy[1]), xy[3]+0.75*(xy[4]-xy[3]),
     xy[1]+0.21*(xy[2]-xy[1]), xy[3]+0.99*(xy[4]-xy[3]),
     col="#ffffffc0", border = "grey50")
symbols(rep(xy[1]+0.1*(xy[2]-xy[1]), 3),
  c(xy[3]+0.9*(xy[4]-xy[3]), xy[3]+0.83*(xy[4]-xy[3]), xy[3]+0.78*(xy[4]-xy[3])),
        add=T, inches=F, circles=ff*sqrt(c(bubhi,bubmd,bublo)),
...
```{r bubb-Al, fig.height=7.2, fig.width=10.2, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Map of Al concentrations by location at Ashfield Flats for all sampling years.", echo=FALSE}
par(oma=c(3,3,1,1), mar=c(0,0,0,0), mgp=c(1.7,0.2,0), tcl=0.25)
plot_tiles(aftiles)
with(afr_map, polygon(wetland_E, wetland_N, border = "steelblue", col="#87CEEB80"))
axis(1);axis(2);box()
...
ff <- (0.025 * (par("usr")[4]-par("usr")[3]))/sqrt(max(afs1923$Al,na.rm=T))
afs1923$bubcats <- cut(afs1923$Al, breaks = brk0)
ppal <- scico::scico(length(brk0)-1, pal="lajolla", alpha=0.5)
...
with(afs1923, symbols(Easting, Northing, add=TRUE, circles = ff*sqrt(Al), inches=F,
     bg=ppal[bubcats], fg="grey40")) # fg=ppal[floor(length(ppal)/1.5)]
...
rect(xy[1]+0.04*(xy[2]-xy[1]), xy[3]+0.75*(xy[4]-xy[3]),
     xy[1]+0.22*(xy[2]-xy[1]), xy[3]+0.99*(xy[4]-xy[3]),
     col="#ffffffc0", border = "grey50")
symbols(rep(xy[1]+0.1*(xy[2]-xy[1]), 3),
  c(xy[3]+0.9*(xy[4]-xy[3]), xy[3]+0.83*(xy[4]-xy[3]), xy[3]+0.78*(xy[4]-xy[3])),
        add=T, inches=F, circles=ff*sqrt(c(bubhi,bubmd,bublo)),
        bg=c(ppal[length(ppal)], ppal[floor(length(ppal)/2)], ppal[1]),
  fg="grey40") # fg=ppal[floor(length(ppal)/3)]
text(rep(xy[1]+0.1*(xy[2]-xy[1]), 3),
     c(xy[3]+0.9*(xy[4]-xy[3]), xy[3]+0.83*(xy[4]-xy[3]), xy[3]+0.78*(xy[4]-xy[3])),
     labels=c(bubhi,bubmd,bublo), pos=4, offset=1.2)
text(xy[1]+0.12*(xy[2]-xy[1]), xy[3]+0.96*(xy[4]-xy[3]),
...
```{r quantile-bubb-Cu-Zn-Pb, fig.height=10, fig.width=14.6, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Map of Al concentrations by location at Ashfield Flats for all sampling years.", echo=FALSE}
par(mfrow=c(2,2), oma=c(3,3,1,1), mar=c(0,0,0,0), mgp=c(1.7,0.2,0), tcl=0.25)
varz=c("Cu","Pb","Zn","REE")
symhues <- scico(6, pal="nuuk", alpha=0.8, dir=-1, begin=0.05)
for(i in 1:length(varz)){
  plot_tiles(aftiles)
  with(afr_map, polygon(wetland_E, wetland_N, border = "steelblue", col="#87CEEB80"))
  if(i %in% c(3,4)) {axis(1); mtext("E (m)",1,1.7,font=2,cex = 1.2)
  } else {
      axis(1, lab=F)
    }
  if(i %in% c(1,3)) {axis(2); mtext("N (m)",2,1.7,font=2,cex = 1.2)
  } else {
      axis(2, lab=F)
    }
  box()
  legend("bottomright", bty="o", cex=0.8,box.col="#00000000", bg="#ffffff40",
       legend="Map tiles: CartoDB Voyager, EPSG:32750")
  addnortharrow(); addscalebar(plotepsg=32750, htin=0.12, label.cex=1.2)
  afs1923$q0 <- cut(afs1923[,varz[i]],
                    breaks=quantile(afs1923[,varz[i]], na.rm=T,
                                    pro=c(0,0.05,0.25,0.5,0.75,0.95,1)),
                                    lab=c("Min-5th","5-25th","25-50th",
                                          "50-75th","75-95th","95th-Max"))
  with(afs1923, points(Easting, Northing, pch=21,
                       bg=symhues[q0], cex=c(0.6,0.9,1.2,1.5,1.9,2.4)[q0]))
  mtext(paste0("(",letters[i],")"), 3, -2, cex=1.25, font=2, col=symhues[6])
legend("topleft", pch=21, legend=levels(afs1923$q0), bty="n", cex=1.05, y.in=1.05,
       pt.bg=symhues, pt.cex=c(0.6,0.9,1.2,1.5,1.9,2.4),
       title = paste(varz[i],"percentiles"), title.cex=1.2, title.col="#003087")
}
```

## ├ Are Cu, Pb, Zn related to Al?

```{r corrplots-Al-REE-traces, fig.height=5, fig.width=10, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Correlation heatmaps for Al, La, Ce, Nd, Gd, Cu, Pb, and Zn, for log-transformed (closed) concentrations (left) and CLR-transformed (non-closed) concentrations (right).", echo=FALSE}
par(mfrow=c(1,2), mar=c(3,3,3,3), oma=c(3,3,3,3), mgp=c(1.7,0.2,0), tcl=0.25)
corrplot::corrplot(cor(log10(afs1923[,c("Al","La","Ce","Nd","Gd","Cu","Pb","Zn")]), 
                       use="pairwise.complete"),diag=F,
                   tl.col=1); mtext("log-transformed",1,3)
corrplot::corrplot(cor(afs1923clr[,c("Al","La","Ce","Nd","Gd","Cu","Pb","Zn")], 
                       use="pairwise.complete"),diag=F,
                   tl.col=1); mtext("CLR-transformed",1,3)
```
```{r corrmats-Al-REE-CuPbZn, echo=FALSE}
cat("==== log10-transformed concentrations ====================\n")
RcmdrMisc::rcorr.adjust(log10(afs1923[,c("Al","La","Ce","Nd","Gd","Cu","Pb","Zn")]), 
                        use="pairwise.complete")
cat("\n==== CLR-transformed concentrations ====================\n")
RcmdrMisc::rcorr.adjust(afs1923clr[,c("Al","La","Ce","Nd","Gd","Cu","Pb","Zn")], 
                        use="pairwise.complete")
```
...
## ├ Individual REE and Al, Ca, Fe, S

```{r ree-boxplots-by-type, fig.height=4, fig.width=5, fig.align='center',fig.cap="Boxplots of ∑REEs by sample type at Ashfield Flats for all sampling years (italic letters below axis labels are unique if a pairwise Conover test p value ≤ 0.05).", echo=FALSE}
afs1923surf$REE <- with(afs1923surf, La+Ce+Nd+Gd)
varz <- c("REE")
par(mfrow = c(1,1), mar = c(3.5,3,0.5,0.5), mgp = c(1.6,0.15,0),
    cex.lab = 1.25, cex.axis=1.2, font.lab = 2, tcl = 0.2, xpd=T)
for(i in 1:length(varz)){
  with(afs1923surf, boxplot(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~ Type, 
                        varwidth=T, names=c("Drain","Pond","Saltmarsh","Other"),
                        xlab="", ylab = paste(varz[i],"(mg/kg)"),
                        col=scico(4,pal="batlow",alpha=0.65, end=0.75)))
  kw0 <- with(afs1923surf, 
              kruskal.test(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~ 
                           Type))
  pwc0 <- with(afs1923surf, 
             kwAllPairsConoverTest(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~
                                     Type))
  mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
  shadowtext(1:4, rep(-70,4), labels=mcl0$Letters, col="#003087", bg="white",
             font=4, cex=1.2, family="serif")
  mtext("Type",1,2.2, font=2, cex=1.2)
}
```

<p>&nbsp;</p>

```{r boxplots-by-type, fig.height=7.5, fig.width=10, fig.align='center',fig.cap="Boxplots of REEs and selected major and trace elements by sample type at Ashfield Flats for all sampling years (DS = Drain sediment; PS = Pond Sediment; SM = Saltmarsh; Oth = Mixed/other). Italic letters above axis labels are unique if a pairwise t-test p value ≤ 0.05.", echo=FALSE}
varz <- c("La","Ce","Gd","Nd","Y","Al","Ca","Fe","S","Pb","Th","Zn")
iflog <- c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,TRUE,FALSE,TRUE,TRUE,FALSE,TRUE)
par(mfrow = c(3,4), mar = c(3,3,0.5,0.5), mgp = c(1.6,0.15,0),
    cex.lab = 1.5, cex.axis=1.2, font.lab = 2, tcl = 0.2)
for(i in 1:length(varz)){
  if(iflog[i]==FALSE){
  with(afs1923surf, boxplot(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~ Type, 
                        varwidth=T, names=c("DS","PS","SM","Oth"),
  ...
  kw0 <- with(afs1923surf, 
              kruskal.test(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~ 
                           Type))
  pwc0 <- with(afs1923surf, 
             pairwise.t.test(afs1923surf[,which(colnames(afs1923surf)==varz[i])],
                                     Type))
  mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
  shadowtext(1:4, rep(min(afs1923surf[,varz[i]], na.rm=T),4), 
             labels=mcl0$Letters, col="#003087", bg="white",
             font=4, cex=1.5, family="serif")
  } else {
  with(afs1923surf, boxplot(log10(afs1923surf[,which(colnames(afs1923surf)==varz[i])]) ~ Type, 
                        varwidth=T, names=c("DS","PS","SM","Oth"),
                        ylab = paste(varz[i],"(mg/kg)"), yaxt="n",
                        col=scico(4,pal="batlow",alpha=0.65, end=0.75)))
    axis(2, at=log10(c(1,2,5,10,20,50,100,200,500,1000,2000,5000,1e4,2e4,5e4)),
         labels=c(1,2,5,10,20,50,100,200,500,1000,2000,5000,1e4,2e4,5e4))
  kw0 <- with(afs1923surf, 
              kruskal.test(afs1923surf[,which(colnames(afs1923surf)==varz[i])] ~ 
                           Type))
  pwc0 <- with(afs1923surf, 
             pairwise.t.test(afs1923surf[,which(colnames(afs1923surf)==varz[i])],
                                     Type))
  mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
  shadowtext(1:4, rep(log10(min(afs1923surf[,varz[i]], na.rm=T)),4), 
             labels=mcl0$Letters, col="#003087", bg="white",
             font=4, cex=1.5, family="serif")
  }
}
...
```{r boxplots-REE-zone, fig.height=7.2, fig.width=12, fig.align='center',fig.cap="Boxplots of REEs by sampling Zone at Ashfield Flats for all sampling years (DS = Drain sediment; Fl = Flooded; PS = Pond Sediment; SM = Saltmarsh; So = Soil).", echo=FALSE}
varz <- c("La","Ce","Gd","Nd")
palette(c("black",scico(4,pal="batlow",alpha=0.65, end=0.75),"grey","white"))
...
for(i in 1:length(varz)){
  with(afs1923, boxplot(afs1923[,which(colnames(afs1923)==varz[i])] ~ Zone, 
      ...
      col = c(2,2,5,3,3,3,5,3,4,4,3,2)))
}
plot(c(0,1),c(0,1),type="n",ann=F,xaxt="n",yaxt="n",bty="n",xlab="",ylab="",xaxs="i")
legend(0,1, bty = "n", inset = 0.01, cex = 1.6, 
       legend = c("Drain","Pond","Saltmarsh","Mixed/other"), pch = 22,
       pt.bg = c(2,3,4,5), pt.cex = 3.6)
text(0,0.32,pos=4,cex=1.2, labels=paste0("CMD = Chapman Drain\n",
      "KMD = Kitchener Drain\n", "FW = Flooded woodland\n",
      "N = North wetland pond\n", "NE = North-east wetland pond\n",
      "NW = North-west wetland pond\n", "S = South wetlands / side drain\n",
      "SE = South-east wetland pond\n", "SM = Saltmarsh (east of CMD)\n", 
...
```{r boxplots-REEY-zone, fig.height=7.2, fig.width=12, fig.align='center',fig.cap="Boxplots of REE+Y by sampling Zone at Ashfield Flats for all sampling years.", echo=FALSE}
layout(matrix(1))
...
plot(c(0,1),c(0,1),type="n",ann=F,xaxt="n",yaxt="n",bty="n",xlab="",ylab="",xaxs="i")
legend(-0.16,1.1, bty = "n", inset = 0, cex = 1.6, 
       legend = c("Drain","Pond","Saltmarsh","Mixed/other"), pch = 22,
       pt.bg = c(2,3,4,5), pt.cex = 3.6)
text(-0.16,0.16,pos=4,cex=0.95, labels=paste0("CMD = Chapman Drain, ",
      "KMD = Kitchener Drain\n", "FW = Flooded woodland, ",
      "N = North wetland pond\n", "NE = North-east wetland pond, ",
      "NW = North-west wetland pond\n", "S = South wetlands / side drain, ",
      "SE = South-east wetland pond\n", "SM = Saltmarsh (east of CMD), ",
...
```{r boxplots-maj-zone, fig.height=7.2, fig.width=12, fig.align='center',fig.cap="Boxplots of selected major elements by sampling Zone at Ashfield Flats for all sampling years (CMD = Chapman Drain, KMD = Kitchener Drain, LP = Limestone path, N = North wetland pond, NE = North-east wetland pond, NW = North-west wetland pond, S = South wetlands and side drain, SE = South-east wetland pond, SM = Saltmarsh (east of CMD), SM_SW = Saltmarsh (southwest of CMD), SW = South-west wetland pond, WD = Woolcock Drain).", results='hold', echo=FALSE}
varz <- c("Al","Ca","Fe","P","S")
...
with(afs1923, boxplot(Al ~ Zone, ylab = "Al (mg/kg)",
     xlab="Zone", cex=1, col=c(2,2,5,3,3,3,5,3,4,4,3,2)))
for(i in 2:length(varz)){
  with(afs1923, boxplot(log10(afs1923[,which(colnames(afs1923)==varz[i])]) ~ 
      ...
      cex=1, col = c(2,2,5,3,3,3,5,3,4,4,3,2)))
...
for(i in 1:length(varz)){
  with(afs1923, boxplot(log10(afs1923[,which(colnames(afs1923)==varz[i])]) ~ 
      Zone, yaxt="n", xlab="Zone", ylab = paste(varz[i],"(mg/kg)"),
      col = c(2,2,5,3,3,3,5,3,4,4,3,2), cex = 1))
...
```{r scatter-plot-matrix-untransf, fig.height=12, fig.width=12, fig.align='center',fig.cap="Scatter plot matrix for log10-transformed elements at Ashfield Flats for all sampling years.", eval=FALSE}
carPalette(palette())
...
```{r scatterplots-sumREE-Al, fig.height=10, fig.width=15, message=FALSE, warning=FALSE, fig.align='center',fig.cap="REE-Al relationships at Ashfield Flats for all sampling years. Plots (a) and (b) are plotted on log~10~-transformed variables, plots (c) and (d) are of untransformed variables.", results='hold', echo=FALSE}
par(mfrow = c(2,2), mar = c(4,4,0.5,0.5), mgp = c(1.7,0.2,0),
    cex.lab = 1.5, cex.axis = 1.25, font.lab = 2, tcl = 0.2, las = 1)
afs1923$Al2 <- afs1923$Al/1e3
clrz <- scico(4, pal="batlow", end=0.75, alp=0.5) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
plot(afs1923$Al/1e3, afs1923$REE,
     log="xy", bg = clrz[afs1923$Type], pch = seq(21,25)[afs1923$Type],
     ylab = "∑REE (mg/kg)", cex = 1.4, xlab = "Al (g/kg)") 
  for(j in 1:nlevels(afs1923$Type)){
    lm0 <- lm(log10(afs1923$REE) ~ log10(afs1923$Al/1e3),
...
mtext("(a)",3,-1.8,font=2,cex=1.4,adj=0.02)
legend("left", bty="n", cex = 1.4, legend=levels(afs1923$Type),
       pch=seq(21,25), pt.bg=clrz, pt.cex=1.8)
clrz <- scico(12, pal="hawaii", end=0.85, alp=0.7) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
plot(afs1923$Al/1e3, afs1923$REE,
     log="xy", bg = clrz[afs1923$Zone], 
     pch = c(21:25,21:25,21,22)[afs1923$Zone],
     ylab = "∑REE (mg/kg)", cex = 1.4, xlab = "Al (g/kg)") 
  for(j in 1:nlevels(afs1923$Zone)){
    lm0 <- lm(log10(afs1923$REE) ~ log10(afs1923$Al/1e3),
              subset = afs1923$Zone == levels(afs1923$Zone)[j])
    v0 <- subset(afs1923$Al/1e3,
                 subset = afs1923$Zone == levels(afs1923$Zone)[j])
    r0 <- log10(range(v0, na.rm=T))
    y0 <- c(lm0$coef[1]+lm0$coef[2]*r0[1],lm0$coef[1]+lm0$coef[2]*r0[2])
...
mtext("(b)",3,-1.8,font=2,cex=1.4,adj=0.02)
legend("left", bty="n", cex = 1.1, legend=levels(afs1923$Zone),
       pch=seq(21,25), pt.bg=clrz, pt.cex=1.4)
clrz <- scico(4, pal="batlow", end=0.75, alp=0.5) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
plot(afs1923$Al2, afs1923$REE,
     bg = clrz[afs1923$Type], pch = seq(21,25)[afs1923$Type],
     ylab = "∑REE (mg/kg)", cex = 1.4, xlab = "Al (g/kg)") 
  for(j in 1:nlevels(afs1923$Type)){
    lm0 <- lm(afs1923$REE ~ afs1923$Al2, 
              subset=afs1923$Type==levels(afs1923$Type)[j])
    v0 <- subset(afs1923$Al2,
                 subset = afs1923$Type == levels(afs1923$Type)[j])
    r0 <- range(v0, na.rm=T)
    y0 <- c(lm0$coef[1]+lm0$coef[2]*r0[1],lm0$coef[1]+lm0$coef[2]*r0[2])
    lines(r0, y0, col = clrz[j], lwd = 3)
  }
mtext("(c)",3,-1.8,font=2,cex=1.4,adj=0.02)
legend("left", bty="n", cex = 1.4, legend=levels(afs1923$Type),
       pch=seq(21,25), pt.bg=clrz, pt.cex=1.8)
clrz <- scico(12, pal="hawaii", end=0.85, alp=0.7) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
plot(afs1923$Al2, afs1923$REE,
     bg = clrz[afs1923$Zone], 
     pch = c(21:25,21:25,21,22)[afs1923$Zone],
     ylab = "∑REE (mg/kg)", cex = 1.4, xlab = "Al (g/kg)") 
  for(j in 1:nlevels(afs1923$Zone)){
    lm0 <- lm(afs1923$REE ~ afs1923$Al2,
...
mtext("(d)",3,-1.8,font=2,cex=1.4,adj=0.02)
legend("left", bty="n", cex = 1.1, legend=levels(afs1923$Zone),
       pch=seq(21,25), pt.bg=clrz, pt.cex=1.4)
```
...
```{r scatterplots-Al, fig.height=7, fig.width=10, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Individual REE-Al relationships at Ashfield Flats for all sampling years.", results='hold', echo=FALSE}
varz <- c("La","Ce","Gd","Nd")
clrz <- scico(4, pal="batlow", end=0.75, alp=0.5) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
par(mfrow = c(2,2), mar = c(4,4,0.5,0.5), mgp = c(1.7,0.2,0),
    cex.lab = 1.5, cex.axis = 1.25, font.lab = 2, tcl = 0.2, las = 1)
...
```{r scatterplots-Fe, fig.height=7, fig.width=10, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Individual REE-Fe relationships at Ashfield Flats for all sampling years.", results='hold', echo=FALSE}
varz <- c("La","Ce","Gd","Nd")
clrz <- scico(4, pal="batlow", end=0.75, alp=0.5)
...
```{r scatterplots-P, fig.height=8.4, fig.width=12, message=FALSE, warning=FALSE, fig.align='center',fig.cap="Individual REE-P relationships at Ashfield Flats for all sampling years.", results='hold', echo=FALSE}
varz <- c("La","Ce","Gd","Nd")
clrz <- scico(4, pal="batlow", end=0.75, alp=0.65)
...
legend("topleft", bty="n", cex = 1.4, legend=levels(afs1923$Type),
       pch=seq(21,25), pt.bg=clrz, y.intersp = 0.8)
```

# Scatter Plots for CLR-transformed variables

## ├ Make clr-tansformed data

```{r make clr-tansformed data, message=FALSE, warning=FALSE, paged.print=FALSE, results='hide', echo=FALSE}
...
```{r spm-clr, fig.height=12, fig.width=12, fig.align='center',fig.cap="Scatter plot matrix of CLR-transformed elements at Ashfield Flats for all sampling years.", results='hold', eval=FALSE}
carPalette(palette())
data0 <- na.omit(afs1923clr[,c("Type","Al","Ca","Fe","P","S","La","Ce","Nd","Gd")])
spm(~Al+Ca+Fe+S+P+La+Ce+Nd+Gd|Type, data = data0, smooth=F, regLine=F,
    col=c("#0000ff80","#00b0b080","#a000c080","#b0b00080"))
```

```{r scatter-plot-matrix-CLR-TE, fig.height=12, fig.width=12, fig.align='center',fig.cap="Scatter plot matrix of CLR-transformed elements at Ashfield Flats for all sampling years.", results='hold', eval=FALSE}
carPalette(palette())
...
```{r clr-sp-Al, fig.height=8.4, fig.width=12, message=FALSE, warning=FALSE, fig.align='center',fig.cap="REE-Al relationships (concentrations CLR-transformed) at Ashfield Flats for all sampling years.", results='hold', echo=FALSE}
varz <- c("La","Ce","Gd","Nd")
clrz <- scico(4, pal="batlow", end=0.75, alp=0.65) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
par(mfrow = c(2,2), mar = c(4,4,0.5,0.5), mgp = c(2.6,0.2,0), lend=2, ljoin=1,
    cex.lab = 1.5, cex.axis = 1.25, font.lab = 2, tcl = 0.2, las = 1)
for(i in 1:length(varz)){
...
## ├ CLR-transformed REE *vs*. Al, Ca, Fe, P

```{r clr-sp-REE-Al-Ca-Fe-P, fig.height=8.4, fig.width=18, message=FALSE, warning=FALSE, fig.align='center',fig.cap="REE-Al relationships (concentrations CLR-transformed) at Ashfield Flats for all sampling years.", results='hold', echo=FALSE}
# first make dataset with just sum-of-REE not individual REE...
afs1923ree <- afs1923[,c(1:18,24:50)]
afs1923ree$REE <- with(afs1923ree, La+Ce+Nd+Gd)
afs1923ree <- 
  afs1923ree[,-which(colnames(afs1923ree) %in% c("La","Ce","Nd","Gd"))]
# ...then CLR-transform it
afs1923clr2ree <- afs1923ree
for(i in 19:42){
  afs1923clr2ree[which(afs1923clr2ree[,i]<1e-6),i] <- 
               rep(NA, length(which(afs1923clr2ree[,i]<1e-6)))
}
...
varz <- c("Al","Ca","Fe","P","S")
clrz <- scico(4, pal="batlow", end=0.75, alp=0.65) # c("#4040FF80","#40c0c080","#b000ff80","#CCA66680")
par(mfrow = c(2,3), mar = c(4,4,0.5,0.5), mgp = c(2,0.2,0), lend=2, ljoin=1,
    cex.lab = 1.5, cex.axis = 1.25, font.lab = 2, tcl = 0.3, las = 1)
for(i in 1:length(varz)){
  with(afs1923clr2ree, plot(REE~afs1923clr2ree[,which(colnames(afs1923clr2ree)==varz[i])], 
                        bg = clrz[Type], pch = seq(21,25)[Type], cex = 1.4,
                     xlab = paste(varz[i],"(clr-transformed)"), cex.lab=2, 
                     ylab = ""))
  mtext("REE (clr-transformed)",2,1.6,font=2,cex = 1.4, las=0)
  for(j in 1:nlevels(afs1923clr2ree$Type)){
    lm0 <- lm(afs1923clr2ree$REE ~ afs1923clr2ree[,which(colnames(afs1923clr2ree)==varz[i])],
              subset = afs1923clr2ree$Type == levels(afs1923clr2ree$Type)[j])
    v0 <- subset(afs1923clr2ree[,which(colnames(afs1923clr2ree)==varz[i])],
              subset = afs1923clr2ree$Type == levels(afs1923clr2ree$Type)[j])
    x0 <- min(v0, na.rm=T); x1 <- max(v0, na.rm=T)
    lines(c(x0,x1), c(lm0$coef[1]+lm0$coef[2]*x0,lm0$coef[1]+lm0$coef[2]*x1), 
          col = clrz[j], lwd = 3)
...
legend("left", bty="n", cex = 2, pt.cex=2, legend=levels(afs1923$Type),
       pch=seq(21,25), pt.bg=clrz, y.intersp = 1.25)
```
...
data0 <- na.omit(afs1923clr[,c("Type","Zone","Al","La")])
lmLaAl0 <- with(data0, lm(La ~ Al)) ; summary(lmLaAl0)
cat(rep("-=-",20),sep="");lmLaAl1 <- with(data0, lm(La ~ Al * Type)) ; summary(lmLaAl1)
cat(rep("-=-",20),sep="");lmLaAl2 <- with(data0, lm(La ~ Al * Zone)) ; summary(lmLaAl2)
cat(rep("-=-",9),"\n");aovLaAlmods <- anova(lmLaAl0,lmLaAl1); print(aovLaAlmods, type="text")
cat(rep("-=-",9),"\n");aovLaAlmods <- anova(lmLaAl0,lmLaAl2); print(aovLaAlmods, type="text")
```

## ├ Ce-Al

```{r lm-Ce-Al-simple-grouped, results='hold', echo=FALSE}
data0 <- na.omit(afs1923clr[,c("Type","Zone","Al","Ce")])
lmCeAl0 <- with(data0, lm(Ce ~ Al)) ; summary(lmCeAl0)
cat(rep("-=-",20),sep="");lmCeAl1 <- with(data0, lm(Ce ~ Al * Type)) ; summary(lmCeAl1)
cat(rep("-=-",20),sep="");lmCeAl2 <- with(data0, lm(Ce ~ Al * Zone)) ; summary(lmCeAl2)
cat(rep("-=-",9),"\n");aovCeAlmods <- anova(lmCeAl0,lmCeAl1); print(aovCeAlmods, type="text")
cat(rep("-=-",9),"\n");aovCeAlmods <- anova(lmCeAl0,lmCeAl2); print(aovCeAlmods, type="text")
```

## ├ Nd-Al

```{r lm-Nd-Al-simple-grouped, results='hold', echo=FALSE}
data0 <- na.omit(afs1923clr[,c("Type","Zone","Al","Nd")])
lmNdAl0 <- with(data0, lm(Nd ~ Al)) ; summary(lmNdAl0)
cat(rep("-=-",20),sep="");lmNdAl1 <- with(data0, lm(Nd ~ Al * Type)) ; summary(lmNdAl1)
cat(rep("-=-",20),sep="");lmNdAl2 <- with(data0, lm(Nd ~ Al * Zone)) ; summary(lmNdAl2)
cat(rep("-=-",9),"\n");aovNdAlmods <- anova(lmNdAl0,lmNdAl1); print(aovNdAlmods, type="text")
cat(rep("-=-",9),"\n");aovNdAlmods <- anova(lmNdAl0,lmNdAl2); print(aovNdAlmods, type="text")
```

## ├ Gd-Al

```{r lm-Gd-Al-simple-grouped, results='hold', echo=FALSE}
data0 <- na.omit(afs1923clr[,c("Type","Zone","Al","Gd")])
lmGdAl0 <- with(data0, lm(Gd ~ Al)) ; summary(lmGdAl0)
cat(rep("-=-",20),sep="");lmGdAl1 <- with(data0, lm(Gd ~ Al * Type)) ; summary(lmGdAl1)
cat(rep("-=-",20),sep="");lmGdAl2 <- with(data0, lm(Gd ~ Al * Zone)) ; summary(lmGdAl2)
cat(rep("-=-",9),"\n");aovGdAlmods <- anova(lmGdAl0,lmGdAl1); print(aovGdAlmods, type="text")
cat(rep("-=-",9),"\n");aovGdAlmods <- anova(lmGdAl0,lmGdAl2); print(aovGdAlmods, type="text")
```

## ├ \u2211~4~REE-Al

```{r lm-REE-Al-simple-grouped, results='hold', echo=FALSE}
data0 <- na.omit(afs1923clr2ree[,c("Type","Zone","Al","REE")])
lmREEAl0 <- with(data0, lm(REE ~ Al)) ; summary(lmREEAl0)
cat(rep("-=-",20),sep="");lmREEAl1 <- with(data0, lm(REE ~ Al * Type)) ; summary(lmREEAl1)
cat(rep("-=-",20),sep="");lmREEAl2 <- with(data0, lm(REE ~ Al * Zone)) ; summary(lmREEAl2)
cat(rep("-=-",9),"\n");aovREEAlmods <- anova(lmREEAl0,lmREEAl1); print(aovREEAlmods, type="text")
cat(rep("-=-",9),"\n");aovREEAlmods <- anova(lmREEAl0,lmREEAl2); print(aovREEAlmods, type="text")
```

From the output we see that a grouped (by Type) model has significantly better
predictive ability than ungrouped, based on ANOVA of nested models generated
using the CLR-transformed concentrations.
...
## ├ Map principal component scores
...
legend("bottomright", bty="o", cex=0.8, y.intersp = 0.5,
  legend=expression(italic("Map tiles: CartoDB Voyager, Projection: UTM Zone 50S, WGS84 (EPSG:32750)")),
       box.col="#00000000", bg="#ffffff40")
addnortharrow()
addscalebar(plotepsg=32750, htin=0.12, label.cex=1.2)
...
```{r make-reference-data, message=FALSE, warning=FALSE, paged.print=FALSE, echo=FALSE, results='hold'}
...
```{r make-PAAS-normalised-data-and-plot, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Concentrations of YREE normalized to PAAS over all sediment sample zones and years at Ashfield Flats. ∑REE = La+Ce+Nd+Gd for both samples and PAAS.", results='hold', echo=FALSE}
...
with(plotdata, boxplot(values ~ ind, xaxt="n",  
                       ...
                       col = viridis::mako(6, dir=-1, begin=0.33)))
points(c(1:6), as.numeric(meanz), pch=3, col="white", lwd=4)
points(c(1:6), as.numeric(meanz), pch=3, lwd=2)
axis(1, at=1:5, labels=c("La","Ce","Gd","Nd","Y"), font=2)
axis(1, at=6, labels=expression(bold(paste(Sigma[4],"REE"))), font=2)
...
```{r, echo=FALSE}
cat("Means for ∑REE/PAAS by element:\n"); print(meanz,digits=3); cat("\n")
with(plotdata, bartlett.test(values~ind))
with(plotdata, oneway.test(values~ind))
(pwREE <- with(plotdata, pairwise.t.test(values, ind, pool.sd = F)))
pwREE_pv <- fullPTable(pwREE$p.value)               # from rcompanion
cat("\nCompact letter comparison\n")
multcompLetters(pwREE_pv)                           # from multcompView
```

<p>&nbsp;</p>

## ├ over all data is ∑REE fractionation different by sample Type?

```{r boxplot-REE-PAAS-by-Type, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Concentrations of YREE normalized to PAAS over all sediment sample zones and years at Ashfield Flats. ∑REE = La+Ce+Nd+Gd for both samples and PAAS. Means are different (p < 0.05, pairwise t-test with corrections for unequal variance and multiple comparisons) if italic text below x-axis labels has no common letters.", results='hold', echo=FALSE}
par(mfrow=c(1,1), mar = c(3,4,1,1), mgp=c(1.7,0.3,0), tcl=0.25, font.lab=2,
    lend=2, ljoin=1, xpd=T)
meanz <- with(afs1923_PAAS, tapply(REE, Type, FUN=mean, na.rm=TRUE))
with(afs1923_PAAS, boxplot(REE ~ Type, #xaxt="n",  
                       xlab="", ylab="Normalized REE = sample/PAAS",
                       col = scico(4, pal="batlow", end=0.67, alpha=0.65)))
pwc0 <- with(afs1923_PAAS, pairwise.t.test(afs1923_PAAS$REE, Type, pool.sd=F))
mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
points(c(1:4), as.numeric(meanz), pch=3, col="white", lwd=4)
points(c(1:4), as.numeric(meanz), pch=3, lwd=2)
abline(h=0.99, lty=3,col="sienna4");abline(h=1, lty=3, col="gold")
text(seq(1,12), rep(-0.5,12), labels = paste0("(",mcl0$Letters,")"),
     col = "blue3", cex = 1, family = "serif", font = 3)
```

<p>&nbsp;</p>

```{r compare-REE-PAAS-by-Type, message=FALSE, warning=FALSE, paged.print=FALSE, results='hold', echo=FALSE}
cat("Means for ∑REE/PAAS by sediment Type:\n"); print(meanz,digits=3); cat("\n")
with(afs1923_PAAS, bartlett.test(REE ~ Type))
with(afs1923_PAAS, oneway.test(REE ~ Type))
(pwPAAS <- with(afs1923_PAAS, pairwise.t.test(REE, Type, pool.sd = F)))
pwPAAS_pv <- fullPTable(pwPAAS$p.value)              # from rcompanion
cat("\nCompact letter comparison\n")
multcompLetters(pwPAAS_pv)                           # from multcompView
```

<p>&nbsp;</p>

## Over all data are REE fractionated by Zone?

```{r boxplot-REE-PAAS-by-Zone, message=FALSE, warning=FALSE, paged.print=FALSE, fig.width=12, fig.height=5.5, fig.cap="Concentrations of YREE normalized to PAAS by Zone, over all and years at Ashfield Flats. ∑REE = La+Ce+Nd+Gd for both samples and PAAS.  Means are different (p < 0.05, pairwise t-test with corrections for unequal variance and multiple comparisons) if italic text above x-axis has no common letters.", results='hold', echo=FALSE}
par(mfrow=c(1,1),mar = c(4.5,4.5,1,1), mgp=c(2.7,0.4,0), tcl=0.25, font.lab=2, 
    xpd=T, las=1, lend=2, ljoin=1)
layout(matrix(c(1,1,1,2),nrow = 1,byrow = T))
palette(c("black",scico(4,pal="batlow",alpha=0.65, end=0.75),"grey","white"))
with(afs1923_PAAS, boxplot(REE ~ Zone, cex.axis=1.6, cex.lab=2.2, cex = 1.5, 
                      col = c(2,2,5,3,3,3,5,3,4,4,3,2), xlab="Sampling Zone",
                      ylab="Normalized \u2211REE = sample/PAAS"))
pwc0 <- with(afs1923_PAAS, pairwise.t.test(afs1923_PAAS$REE, Zone, pool.sd=F))
mcl0 <- multcompLetters(fullPTable(pwc0$p.value))
...
```{r compare-PAAS-REE-zone, message=FALSE, warning=FALSE, paged.print=FALSE, results='hold', echo=FALSE}
with(afs1923_PAAS, oneway.test(REE ~ Zone))
pwPAAS <- with(afs1923_PAAS, pairwise.t.test(REE, Zone, pool.sd = F))
cat("Pairwise",pwPAAS$method,"on", pwPAAS$data.name,"with",pwPAAS$p.adjust.method,"correction\n")
print(round(pwPAAS$p.value,4))
pwPAAS_pv <- fullPTable(pwPAAS$p.value)              # from rcompanion
# mc0 <- multcompLetters(pwPAAS_pv)     # from multcompView
rslt <- 
  data.frame(mean=round(tapply(afs1923_PAAS$REE, afs1923_PAAS$Zone, mean, na.rm=T),2),
      cld=as.vector(multcompLetters(pwPAAS_pv)$Letters))
cat("------------\n");t(rslt)
```

Based on these comparisons it seems there is some fractionation. 

<p>&nbsp;</p>

## ├ are REE significantly different from PAAS?

```{r REE-PAAS-ssttests, echo=FALSE}
...
```
```{r, echo=FALSE}
cat("Means for each element in Drain sediment:\n"); print(meanz,digits=3)
with(plotdata, oneway.test(values ~ ind))
(pwREE <- with(plotdata, pairwise.t.test(values, ind, pool.sd = F)))
pwREE_pv <- fullPTable(pwREE$p.value)               # from rcompanion
multcompLetters(pwREE_pv)                           # from multcompView
...
## ├ REE/PAAS in Pond sediments

```{r normalized-REE-Pond-sed, fig.height=3.6, fig.width=6, fig.cap="Concentrations of YREE normalized to PAAS in pond (seasonal wetland) sediments over all years at Ashfield Flats.", results='hold', echo=FALSE}
require(viridis)
PAAS_PondSed <- subset(afs1923_PAAS, afs1923_PAAS$Type=="Pond_Sed")
plotdata <- 
  stack(st_drop_geometry(PAAS_PondSed[,c("La","Ce","Gd","Nd","Y","REE")]))
par(mfrow=c(1,1), mar = c(2,4,1,1), mgp=c(1.7,0.3,0), tcl=0.25, font.lab=2,
    lend=2, ljoin=1)
meanz <- with(plotdata, tapply(values, ind, FUN=mean, na.rm=TRUE))
with(plotdata, boxplot(values ~ ind, xaxt="n", xlab="", 
...
Wetland pond ("Pond") sediments are enriched relative to PAAS. Y is significantly less than La, Ce, Nd, and Gd.

```{r, echo=FALSE}
cat("Means for each element in Pond sediment:\n"); print(meanz,digits=3)
with(plotdata, oneway.test(values ~ ind))
(pwREE <- with(plotdata, pairwise.t.test(values, ind, pool.sd = F)))
pwREE_pv <- fullPTable(pwREE$p.value)               # from rcompanion
multcompLetters(pwREE_pv)                           # from multcompView
...
```{r, echo=FALSE}
cat("Means for each element in Saltmarsh sediment:\n"); print(meanz,digits=3)
with(plotdata, oneway.test(values ~ ind))
(pwREE <- with(plotdata, pairwise.t.test(values, ind, pool.sd = F)))
pwREE_pv <- fullPTable(pwREE$p.value)               # from rcompanion
multcompLetters(pwREE_pv)                           # from multcompView
...
```{r normalized-REE-flooded, fig.height=3.6, fig.width=6, fig.cap="Concentrations of YREE normalized to PAAS in other (e.g. seasonally-flooded woodland) sediments over all years at Ashfield Flats.", results='hold', echo=FALSE}
require(viridis)
...
```{r, echo=FALSE}
cat("Means for each element in Other sediment:\n"); print(meanz,digits=3)
with(plotdata, oneway.test(values ~ ind))
(pwREE <- with(plotdata, pairwise.t.test(values, ind, pool.sd = F)))
pwREE_pv <- fullPTable(pwREE$p.value)               # from rcompanion
multcompLetters(pwREE_pv)                           # from multcompView
...
mtext(expression(paste("Estimated sulfate:chloride ratio \u2212 log"[10],
                       "((S\u00D7(96/32))/(Na \u00D7 1.799))")), 
      side=1, line=1.6, font=2, cex=1.1)
abline(v=log10(0.14), col = "darkcyan", lty = 2, lwd = 1)
text(log10(0.14) ,5, pos=2, col = "darkcyan", cex = 0.95, offset = 0.15,
     labels=expression(paste("Mean marine ratio: SO"[4]^"2\u2212",
                             "/Cl"^"\u2212","= 0.14")))
abline(v=log10(0.5), col = "magenta4", lty = 2, lwd = 2)
text(log10(0.5) ,7.2, pos=4, col = "magenta4", offset=0.15,
  labels = expression(paste("Acid sulfate trigger")))
text(log10(0.5) ,6, pos=4, col = "magenta4", offset=0.15,
  labels = expression(paste("SO"[4]^"2\u2212","/Cl"^"\u2212","= 0.5")))
...
flextable(afwater1923fresh[which(((afwater1923fresh$S*(96/32))/
  ...
  padding(padding=2) |>
  flextable::set_caption("Identity and ASS-related properties of Ashfield Flats fresh water samples with SO\u2084/Cl > 0.5")
```
